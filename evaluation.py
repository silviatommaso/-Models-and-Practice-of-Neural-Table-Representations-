...
def evaluate(pred, gt):
    ...
    for p_item, g_item in zip(pred, gt):
        ...
        pred_llama = [row for row in (p_item.get("llama") or [])]
        pred_gpt = [row for row in (p_item.get("gpt") or [])]
        gt_pred = [row for row in (g_item.get("prediction") or [])]
        ...
        pred_llama_cells = [str(c) for row in pred_llama for c in row]
        pred_gpt_cells = [str(c) for row in pred_gpt for c in row]
        gt_cells = [str(c) for row in gt_pred for c in row]
        ...
        if llama_empty_correct:
            ...
            cell_llama_precision = (
                correct_llama_cells / len(pred_llama_cells)
                if pred_llama_cells else 0
            )

            cell_llama_recall = (
                correct_llama_cells / len(gt_cells)
                if gt_cells else 0
            )
        ...
        if gpt_empty_correct:
            ...
            cell_gpt_precision = (
                correct_gpt_cells / len(pred_gpt_cells)
                if pred_gpt_cells else 0
            )

            cell_gpt_recall = (
                correct_gpt_cells / len(gt_cells)
                if gt_cells else 0
            )
        ...
        pred_llama_counter = Counter(
            row_to_frozenset(row)
            for row in pred_llama
        )

        pred_gpt_counter = Counter(
            row_to_frozenset(row)
            for row in pred_gpt
        )

        gt_counter_tuples = Counter(
            row_to_frozenset(row)
            for row in gt_pred
        )

        correct_llama_tuples = sum(
            (pred_llama_counter & gt_counter_tuples).values()
        )

        correct_gpt_tuples = sum(
            (pred_gpt_counter & gt_counter_tuples).values()
        )
        ...
        if llama_empty_correct:
            ...
            tuple_llama_constraint = (
                correct_llama_tuples / len(pred_llama)
                if pred_llama else 0
            )

            tuple_llama_cardinality = (
                min(len(pred_llama), len(gt_pred)) /
                max(len(pred_llama), len(gt_pred))
        ...
        if gpt_empty_correct:
            ...
            tuple_gpt_constraint = (
                correct_gpt_tuples / len(pred_gpt)
                if pred_gpt else 0
            )

            tuple_gpt_cardinality = (
                min(len(pred_gpt), len(gt_pred)) /
                max(len(pred_gpt), len(gt_pred))
        ...
        llama_query_score = (
            sum(llama_components) / len(llama_components)
            if llama_components else 0
        )

        gpt_query_score = (
            sum(gpt_components) / len(gpt_components)
            if gpt_components else 0
        )
